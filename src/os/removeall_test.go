...
func TestRemoveAllLongPath(t *testing.T) {
	...
	if runtime.GOOS != "linux" && runtime.GOOS != "darwin" {
		t.Skip("skipping for non-linux/darwin")
	}
	...
	for i := 0; i < 41; i++ {
		...
		err = Mkdir(name, 0755)
		if err != nil {
			t.Fatalf("Could not mkdir %s in %s: %s", name, cwd, err)
		}
...
func TestRemoveAllDot(t *testing.T) {
	tempDir, err := ioutil.TempDir("", "TestRemoveAllLongPath-")
	if err != nil {
		t.Fatalf("Could not create TempDir: %s", err)
	}
	err = Chdir(tempDir)
